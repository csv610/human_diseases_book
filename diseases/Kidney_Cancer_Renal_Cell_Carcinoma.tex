\chapter{Kidney Cancer (Renal Cell Carcinoma)}
\index{Kidney Cancer (Renal Cell Carcinoma)}
...
\begin{itemize}[noitemsep]
  \item You may experience the triad of hematuria, flank pain, and palpable abdominal mass (now uncommon
  \item 60\% are found incidentally on imaging)
  \item Paraneoplastic syndromes include erythrocytosis (EPO), hypercalcemia (PTHrP), high blood pressure (renin), and Stauffer syndrome (reversible liver dysfunction).
...
\begin{itemize}[noitemsep]
  \item Severe or worsening symptoms of kidney cancer (renal cell carcinoma) require immediate medical evaluation
\end{itemize}
...
\begin{itemize}[noitemsep]
  \item Treatment options include partial nephrectomy (nephron-sparing for T1a, T1b tumors), radical nephrectomy for larger tumors, and ablative therapies for small tumors in elderly patients
  \item Advanced disease doctors typically treat it with tyrosine kinase inhibitors (sunitinib, pazopanib, cabozantinib), immunotherapy (nivolumab + ipilimumab, pembrolizumab + axitinib), mTOR inhibitors (everolimus, temsirolimus), and HIF-2$\alpha$ inhibitors (belzutifan for VHL-associated RCC). Recovery varies by stage: 5-year survival is 93\% for localized, 72\% regional, and 14\% metastatic.
\end{itemize}
...
\section{Diagnosis}
\begin{itemize}[noitemsep]
  \item Doctors diagnose this based on CT abdomen with contrast (heterogeneous, enhancing kidney mass), MRI for cystic/complex lesions, and kidney mass biopsy for small tumors
  \item Staging uses the TNM system.
